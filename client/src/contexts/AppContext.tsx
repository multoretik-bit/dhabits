...
export interface Habit {
  ...
  status?: 'implemented' | 'implementing' | 'planned';
}
...
export interface IdentityValueFolder {
  id: string;
  name: string;
  color?: string;
}
...
export interface IdentitySystemIdea {
  id: string;
  aspectId: string;
  folderId?: string;
  text: string;
...
export interface ActivityMicroGoal {
  id: string;
  title: string;
...
interface AppContextType {
  ...
  identityValueFolders: IdentityValueFolder[];
  addIdentityValue: (text: string, folderId?: string) => void;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [activityMicroGoals, setActivityMicroGoals] = useState<ActivityMicroGoal[]>([]);
  const [activityTimer, setActivityTimer] = useState<ActivityTimerState>({ ...DEFAULT_ACTIVITY_TIMER });
  ...
  const [identitySystemFolders, setIdentitySystemFolders] = useState<IdentitySystemFolder[]>([]);
  const [identitySystemIdeas, setIdentitySystemIdeas] = useState<IdentitySystemIdea[]>([]);
  ...
  useEffect(() => {
    ...
    setActivitySessions(savedData.activitySessions || []);
    setActivityMicroGoals(savedData.activityMicroGoals || []);
    setActivityTimer({ ...DEFAULT_ACTIVITY_TIMER, ...(savedData.activityTimer || {}) });
    ...
    setIdentitySystemFolders(savedData.identitySystemFolders || []);
    setIdentitySystemIdeas(savedData.identitySystemIdeas || []);
        ...
        try {
          ...
          if (remoteData) {
            ...
            if (isLocalEmpty || !localLastUpdated || (remoteLastUpdated && new Date(remoteLastUpdated) > new Date(localLastUpdated))) {
              ...
              setActivitySessions(remoteData.activitySessions || []);
              setActivityMicroGoals(remoteData.activityMicroGoals || []);
              setActivityTimer({ ...DEFAULT_ACTIVITY_TIMER, ...(remoteData.activityTimer || {}) });
            ...
            (payload: any) => {
              ...
              if (newData && newData.lastUpdated) {
                ...
                if (remoteTime > localTime) {
                  ...
                  setActivitySessions(newData.activitySessions || []);
                  setActivityMicroGoals(newData.activityMicroGoals || []);
                  setActivityTimer({ ...DEFAULT_ACTIVITY_TIMER, ...(newData.activityTimer || {}) });
  ...
  const saveAllData = (
    ...
    const data: StorageData = {
      ...
      identityValueFolders: identityValueFoldersValue || identityValueFolders,
  ...
  const addGoalFolder = (folder: GoalFolder) => {
    ...
    saveAllData(coins, habits, blocks, habitFolders, goals, newFolders, shopItems, shopFolders, characterState, tasks, taskFolders, customColors);
  };
  ...
  const addShopItem = (item: ShopItem) => {
    ...
    saveAllData(coins, habits, blocks, habitFolders, goals, goalFolders, newItems, shopFolders, characterState, tasks, taskFolders, customColors);
  };
  ...
  const purchaseItem = (id: string): boolean => {
    ...
    setCoins(newCoins);
    setShopItems(newItems);
    saveAllData(newCoins, habits, blocks, habitFolders, goals, goalFolders, newItems, shopFolders, characterState, tasks, taskFolders, customColors);
    return true;
  };
  ...
  const addShopFolder = (folder: ShopFolder) => {
    ...
    saveAllData(coins, habits, blocks, habitFolders, goals, goalFolders, shopItems, newFolders, characterState, tasks, taskFolders, customColors);
  };
  ...
  const completeTask = (id: string, dateStr?: string) => {
    ...
    if (coinsChange !== 0) {
      ...
      setCoins(newCoins);
      setTasks(newTasks);
      saveAllData(newCoins, habits, blocks, habitFolders, goals, goalFolders, shopItems, shopFolders, characterState, newTasks, taskFolders, customColors);
    } else {
      setTasks(newTasks);
      saveAllData(coins, habits, blocks, habitFolders, goals, goalFolders, shopItems, shopFolders, characterState, newTasks, taskFolders, customColors);
    }
  ...
  const setWakeUpTime = (dateStr: string, timeStr: string) => {
    ...
    saveAllData(coins, habits, blocks, habitFolders, goals, goalFolders, shopItems, shopFolders, characterState, tasks, taskFolders, customColors, newTimes);
  };
  ...
  const addActivityMicroGoal = (goal: ActivityMicroGoal) => {
    const newGoals = [...activityMicroGoals, goal];
  ...
  const saveActivityTimer = (timer: ActivityTimerState) => {
  ...
  const deleteIdentityValueFolder = (id: string) => {
    const newFolders = identityValueFolders.filter(f => f.id !== id);
