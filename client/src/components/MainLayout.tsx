      ...
      <header className="nav-blur px-3 py-3 shadow-sm flex items-center justify-between gap-2 overflow-hidden">
        <div className="flex items-center gap-2 flex-shrink-0">
          <Link href="/" className="flex items-center gap-2 flex-shrink-0">
            <img src="/logo.png" alt="dHabits Logo" className="w-6 h-6 sm:w-7 sm:h-7 rounded-[6px] sm:rounded-[8px] object-contain drop-shadow-[0_0_8px_rgba(99,102,241,0.5)]" />
            <span className="font-black text-lg sm:text-xl text-slate-50 tracking-tighter hidden xs:inline-block">dHabits</span>
          </Link>

          <Link href="/identity">
            <button
              className="flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 hover:bg-indigo-500/20 transition-all group"
            >
              <Target className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-indigo-300 group-hover:scale-110 transition-transform" />
              <span className="text-[10px] sm:text-xs font-black text-indigo-100 uppercase tracking-tight hidden xxs:inline-block">Identity</span>
            </button>
          </Link>
        </div>

        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          <div className="flex items-center gap-1.5 sm:gap-2 bg-indigo-950/40 border border-indigo-900/30 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full shadow-inner overflow-hidden max-w-[100px] sm:max-w-none">
            ...
            <div className="relative flex items-center justify-center flex-shrink-0">
              <Cloud className={`w-3 h-3 sm:w-3.5 sm:h-3.5 transition-all duration-300 ${
                ...
                isSyncing ? "text-indigo-400 animate-pulse scale-110" :
          ...
          <Link href="/settings">
            <button
              title="Настройки"
              className="flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-xl text-slate-400 hover:text-indigo-300 hover:bg-indigo-400/10 transition-all border border-transparent"
            >
              ...
              <Link key={path} href={path}>
                <div
                  className={`relative flex flex-col items-center gap-1.5 px-3 py-2 rounded-2xl cursor-pointer min-w-[64px]
                    ${isActive ? "text-indigo-300" : "text-slate-500 hover:text-indigo-300"}`}
                >
                  {isActive && (
                    <motion.div
                      layoutId="nav-active-pill"
                      transition={spring.snappy}
                      className="absolute inset-0 rounded-2xl bg-indigo-950/40 border border-indigo-500/20 shadow-[0_0_16px_rgba(99,102,241,0.15)]"
                    />
                  )}
